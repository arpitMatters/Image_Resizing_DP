...
class SeamEnergyWithBackPointer:
    #we store x coordinate for the pixel of the previous row that led to this particular seam energy
    #and we store total seam energy at the current coordinate. Notice we dont need to store current coordinate since at curr coordinate this class instance will be stored hence can be inferred.
     
    def __init__(self, energy, x_coordinate_in_previous_row = None):    #default is None for the first row.
        self.energy = energy
...
def compute_vertical_seam_v2(energy_data):
    ...
    #gives the entire coordinate of the lowest seam in the image with the help of back pointers

    m_grid = [[None for _ in row] for row in energy_data]    #creating new 2D grid of image size n init to 0

    h = len(energy_data)  #finding height n width of image since image is stored as list of lists
    w = len(energy_data[0])

    for x in range(w):
        m_grid[0][x] = SeamEnergyWithBackPointer(energy_data[0][x])       #wrapping m_grid with instance of the SeamEnergWithBackPointer class (top row)

    for y in range(1, h):           #since top is done so 1 to h
        for x in range(w):
            x_min = x-1 if x>0 else 0
            x_max = x+1 if x<w-1 else w-1           #checking for upper-left, upper-right since if x at left boundary then upper n upper-right is valid.

            min_x_parent = min(range(x_min, x_max+1), key=lambda x_candidate: m_grid[y-1][x_candidate].energy)

            m_grid[y][x] = SeamEnergyWithBackPointer(energy_data[y][x] + m_grid[y-1][min_x_parent].energy, min_x_parent)
